...
export class UnitService {
  ...
  datatable(dt_params){
    ...
    return this.http.get<any>(`${environment.apiUrl}/dt/units/`,
    {
      params: params,
    })
    .pipe(
  ...
  list(filters, pageSize = 5, page = 1, paginate = false): Observable<any> {
    ...
    return this.http.get(`${environment.apiUrl}/units/` + url_segment)
      .pipe(
  ...
  add (data): Observable<any> {
    return this.http.post<any>(`${environment.apiUrl}/units/`, data)
      .pipe(
        tap(_ => this.log('add')),
  ...
  edit (id, data): Observable<any> {
    return this.http.put<any>(`${environment.apiUrl}/units/`+id+"/", data)
      .pipe(
        tap(_ => this.log('edit')),
  ...
  removeImage (id): Observable<any> {
    return this.http.delete<any>(`${environment.apiUrl}/remove-image/`+id)
      .pipe(
        tap(_ => this.log('removeImage')),
        catchError(this.handleError('removeImage', []))
      );
  }
